...
def CEL(predicted: Tensor, actual: Tensor, module: Module = None) -> Tensor:
    ...
    data = (1./m) * (-np.dot(actual.data, np.log(predicted.data).T) - np.dot(1 - actual.data, np.log(1-predicted.data).T))
    requires_grad = predicted.requires_grad

    if requires_grad:
        def grad_fn(grad: np.ndarray) -> np.ndarray:
            return grad * ( - ( np.divide(actual.data, predicted.data) - np.divide(1-actual.data, 1-predicted.data) ))
